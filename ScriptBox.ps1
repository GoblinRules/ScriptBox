...
$script:Catalog = @(
    ...
    New-CatalogItem -Id 'disable-machine-audio' -Name 'Disable Machine Audio' -Category 'Windows' -Description 'Silences physical and Remote Desktop playback for every user until an administrator manually restores it.' -ScriptPath 'Disable-MachineAudio.ps1' -Impact 'Disables and stops Windows Audio services and blocks RDP audio/video playback redirection by machine policy. This persists after restart and requires manual administrator action to reverse.' -RequiresAdmin $true -Accent '#F472B6' -SuccessMessage 'Physical and RDP audio playback are disabled and will remain disabled after restart.'
